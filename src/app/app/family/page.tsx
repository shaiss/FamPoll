  ...
  return (
    <Screen>
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="text-sm text-ink-2">Everyone in this group can vote on its events.</p>
      </div>
      ...
      {organizer && invitable.length > 0 ? (
        <Card className="flex flex-col gap-3 p-4">
          <SectionLabel>Add from your other groups</SectionLabel>
          <p className="text-xs text-ink-3">People you already share a group with. Adding them gives them a seat here straight away — no invite link needed.</p>
            ...
            {invitable.map((u) => (
              <div key={u.userId} className="flex items-center gap-3">
                ...
                <div className="flex min-w-0 flex-1 flex-col">
                  ...
                  <div className="truncate text-xs text-ink-2">In {u.groups.join(", ")}</div>
                </div>
      ...
      {organizer ? (
        <details className="rounded-card border border-line bg-card p-4">
          <summary className="cursor-pointer list-none text-xs font-semibold text-ink-3 [&::-webkit-details-marker]:hidden">Delete this group…</summary>
