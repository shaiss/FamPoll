    ...
    <Screen>
            ...
            <Card key={m.id} className="flex items-center gap-3 p-3">
              ...
              <div className="flex min-w-0 flex-1 flex-col">
                ...
                <div className="text-xs text-ink-2">
                  {m.role === "organizer" ? "Organizer" : proxy ? "No account · someone votes for them" : "Member"}
                </div>
              </div>
              {proxy ? (
                (managedByMe || organizer) ? (
                  <form action={removeProxyMember}>
      ...
      <Card className="p-4">
        <form action={addProxyMember} className="flex flex-col gap-3">
          <Field label="Add someone without a phone" hint="A kid or a grandparent. You cast their vote from your screen, and it counts like everyone else’s.">
