...
A **decision** belongs to an **event** and is described by three choices. Its **format** says what an option is: a short line of text, a longer paragraph, or a date range (exactly one per decision). Its **vote type** says how each voting round is voted: A or B (exactly two options, pick one; always a single round, and nobody can add or remove an option), multiple choice (pick one of several), or pick several (each person picks up to N, never all of them). Its **plan** says how many rounds it takes: a quick vote (one final round), a shortlist then a final, or ideas then shortlist then final. In an **ideas** round people add options and nobody votes. In a **shortlist** round everyone votes and the top two advance (everyone tied at the cut line advances; options with zero votes drop). In a **final** the option with the most votes wins. A round closes when its deadline passes or as soon as every seat in the family has voted, and the next round opens by itself. A tied final asks the organizer to run a tiebreak between the tied options or just call it. The organizer can always close a round early, give it more time, reopen the last round, decide by hand, or set a decision aside. Kids and relatives without a phone get a **proxy seat** that an adult votes from. While a round is open nobody sees anyone else's picks, organizers included; only who has voted so far. Results appear when the round closes, by name unless someone hid their ballot, in which case that round shows counts only. Anyone can hide a ballot (per ballot, or as a standing preference on their seat) and can ask a question or suggest an idea anonymously; every vote is still recorded under its seat.
